...
class FeatureEngineering:
    """Class to perform feature engineering on the dataset."""
    ...
    def apply_lowpass_filter(self, df, columns, fs, cutoff, order=5):
    ...
    def apply_pca(self, df, columns, n_components):
    ...
    def calculate_sum_of_squares(self, df):
    ...
    def apply_temporal_abstraction(self, df, columns, window_size):
    ...
    def apply_frequency_abstraction(self, df, columns, window_size, fs):
    ...
    def perform_clustering(self, df, columns, n_clusters):
    ...
    def visualize_clusters(self, df, columns):
    ...
    def visualize_labels(self, df, columns):
    ...
    def build_new_features(self):
        """Build new features from the dataset."""
        # Impute missing values
        df = self.impute_missing_values(self.df, PREDICTOR_COLUMNS)

        # Calculate set duration
        df = self.calculate_set_duration(df)

        # Apply lowpass filter
        fs = 1000 / 200
        cutoff = 1.3
        df = self.apply_lowpass_filter(df, PREDICTOR_COLUMNS, fs, cutoff)

        # Apply PCA
        df = self.apply_pca(df, PREDICTOR_COLUMNS, n_components=3)

        # Calculate sum of squares
        df = self.calculate_sum_of_squares(df)
        ...
        df = self.apply_frequency_abstraction(
        ...
        # Perform clustering
        df = self.perform_clustering(df, ["acc_x", "acc_y", "acc_z"], n_clusters=5)

        # Visualize clusters
        #self.visualize_clusters(df, ["acc_x", "acc_y", "acc_z"])

        # Visualize labels
        #self.visualize_labels(df, ["acc_x", "acc_y", "acc_z"])

        # Export dataset
        df.to_pickle(self.output_path)
        print(f"Data with features saved to {self.output_path}")
...
def main():
    ...
    """
    fe = FeatureEngineering(DATA_PATH, OUTPUT_PATH)
    fe.build_new_features()
    # Load data
